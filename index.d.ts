...
export type IChatMessage<TMessage extends {}> = TMessage & {
  _id: any;
  text: string;
  createdAt: Date;
  user: {
...
export type ISystemMessage<TMessage extends {}> = TMessage & {
  _id: any;
  text: string;
  createdAt: Date;
  system: true;
...
export type IMessage<T> = IChatMessage<T> | ISystemMessage<T>;
...
interface AvatarProps<T = {}> {
  ...
  position: "left" | "right";
  currentMessage: IChatMessage<T>;
  previousMessage: IMessage<T>;
  nextMessage: IMessage<T>;
  onPressAvatar(): void;
  renderAvatar(props: AvatarProps<T>): JSX.Element;
  containerStyle: {
    left: any;
    right: any;
  };
  ...
  isSameDay(currentMessage: IMessage<T>, message: IMessage<T>): boolean;
  isSameUser(currentMessage: IMessage<T>, message: IMessage<T>): boolean;
}
...
interface BubbleProps<T = {}> {
  ...
  renderTicks?(currentMessage: IMessage<T>): React.ReactNode;
  position?: "left" | "right";
  currentMessage?: IMessage<T>;
  nextMessage?: IMessage<T>;
  previousMessage?: IMessage<T>;
  ...
  // TODO: remove in next major release
  isSameDay?(currentMessage: IMessage<T>, nextMessage: IMessage<T>): boolean;
  isSameUser?(currentMessage: IMessage<T>, nextMessage: IMessage<T>): boolean;
}
...
export interface GiftedChatProps<T = {}> {
  /* Messages to display */
  messages?: IMessage<T>[];
  ...
  messageIdGenerator?(message: IMessage<T>): string;
  /* User sending the messages: { _id, name, avatar } */
  user?: User;
  /* Callback when sending a message */
  onSend?(messages: IMessage<T>[]): void;
  ...
  renderMessage?(message: IMessage<T>): React.ReactNode;
  /* Custom message text */
...
interface MessageContainerProps<T = {}> {
  messages?: IMessage<T>[];
  user?: User;
...
interface SystemMessageProps<T = {}> {
  currentMessage?: IMessage<T>;
  containerStyle?: ViewStyle;
  wrapperStyle?: ViewStyle;
  textStyle?: TextStyle;
...
export type utils<T = {}> = {
  isSameUser(currentMessage?: IMessage<T>, message?: IMessage<T>): boolean;
  isSameDay(currentMessage?: IMessage<T>, message?: IMessage<T>): boolean;
  isSameTime(currentMessage?: IMessage<T>, message?: IMessage<T>): boolean;
};
