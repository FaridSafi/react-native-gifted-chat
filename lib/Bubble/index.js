...
export const Bubble = (props) => {
    const { currentMessage, nextMessage, position, containerToNextStyle, previousMessage, containerToPreviousStyle, onQuickReply, renderQuickReplySend, quickReplyStyle, quickReplyTextStyle, quickReplyContainerStyle, containerStyle, wrapperStyle, bottomContainerStyle, onPressMessage: onPressMessageProp, onLongPressMessage: onLongPressMessageProp, } = props;
    ...
    const renderMessageText = useCallback(() => {
        if (currentMessage?.text) {
            const { 
            /* eslint-disable @typescript-eslint/no-unused-vars */
            containerStyle, wrapperStyle, messageTextProps, 
            /* eslint-enable @typescript-eslint/no-unused-vars */
            ...messageTextPropsRest } = props;
            const combinedProps = {
    ...
    const renderTicks = useCallback(() => {
        ...
        if (currentMessage &&
            ...
            return (<View style={styles.messageStatusContainer}>
          {!!currentMessage.sent && (<Text style={[styles.messageStatus, props.tickStyle]}>
              {'✓'}
            </Text>)}
          {!!currentMessage.received && (<Text style={[styles.messageStatus, props.tickStyle]}>
              {'✓'}
            </Text>)}
          {!!currentMessage.pending && (<Text style={[styles.messageStatus, props.tickStyle]}>
              {'🕓'}
            </Text>)}
        </View>);
        return null;
    ...
    const renderUsername = useCallback(() => {
        ...
        if (props.isUsernameVisible && currentMessage) {
            ...
            return (<View style={styles.usernameContainer}>
    ...
    return (<View style={containerStyle?.[position]}>
      <View style={[
        ...
        <Pressable onPress={onPress} onLongPress={onLongPress} {...props.touchableProps}>
          {renderBubbleContent()}
          <View style={[
            styles.bottom,
