...
export const MessagesContainer = (props) => {
    ...
    const ScrollToBottomWrapper = useCallback(() => {
        ...
        if (!isScrollToBottomVisible)
            return null;
        return (<Pressable style={styles.scrollToBottom} onPress={handleScrollToBottomPress}>
    ...
    return (<View style={[
      ...
      <AnimatedFlatList ref={forwardRef} keyExtractor={keyExtractor} data={messages} renderItem={renderItem} inverted={isInverted} automaticallyAdjustContentInsets={false} style={stylesCommon.fill} ListEmptyComponent={renderChatEmpty} ListFooterComponent={isInverted ? ListHeaderComponent : ListFooterComponent} ListHeaderComponent={isInverted ? ListFooterComponent : ListHeaderComponent} scrollEventThrottle={1} onEndReached={onEndReached} onEndReachedThreshold={0.1} keyboardDismissMode='interactive' keyboardShouldPersistTaps='handled' {...listProps} onScroll={scrollHandler} onLayout={onLayoutList} CellRendererComponent={renderCell}/>
      <ScrollToBottomWrapper />
      {isDayAnimationEnabled && (<DayAnimated scrolledY={scrolledY} daysPositions={daysPositions} listHeight={listHeight} renderDay={renderDayProp} messages={messages} isLoading={loadEarlierMessagesProps?.isLoading ?? false}/>)}
    </View>);
};
