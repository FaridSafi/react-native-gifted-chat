...
- **time format`** _(String)_ - Format to use for rendering times; default is `'LT'`
...
- **load earlier`** _(Bool)_ - Enables the "load earlier messages" button
...
- **render avatar`** _(Function)_ - Custom message avatar; set to `null` to not render any avatar for the message
